...
@rem ##########################################################################
@rem
@rem  stresstest-client startup script for Windows
@rem
@rem ##########################################################################
...
@rem Add default JVM options here. You can also use JAVA_OPTS and STRESSTEST_CLIENT_OPTS to pass JVM options to this script.
set DEFAULT_JVM_OPTS="-verbose:gc" "-XX:+PrintFlagsFinal"
...
@rem Execute stresstest-client
"%JAVA_EXE%" %DEFAULT_JVM_OPTS% %JAVA_OPTS% %STRESSTEST_CLIENT_OPTS%  -classpath "%CLASSPATH%" io.grpc.testing.integration.StressTestClient %CMD_LINE_ARGS%
...
:fail
rem Set variable STRESSTEST_CLIENT_EXIT_CONSOLE if you need the _script_ return code instead of
rem the _cmd.exe /c_ return code!
if  not "" == "%STRESSTEST_CLIENT_EXIT_CONSOLE%" exit 1
exit /b 1
