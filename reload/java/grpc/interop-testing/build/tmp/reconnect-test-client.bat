...
@rem ##########################################################################
@rem
@rem  reconnect-test-client startup script for Windows
@rem
@rem ##########################################################################
...
@rem Add default JVM options here. You can also use JAVA_OPTS and RECONNECT_TEST_CLIENT_OPTS to pass JVM options to this script.
...
@rem Execute reconnect-test-client
"%JAVA_EXE%" %DEFAULT_JVM_OPTS% %JAVA_OPTS% %RECONNECT_TEST_CLIENT_OPTS%  -classpath "%CLASSPATH%" io.grpc.testing.integration.ReconnectTestClient %CMD_LINE_ARGS%
...
:fail
rem Set variable RECONNECT_TEST_CLIENT_EXIT_CONSOLE if you need the _script_ return code instead of
rem the _cmd.exe /c_ return code!
if  not "" == "%RECONNECT_TEST_CLIENT_EXIT_CONSOLE%" exit 1
exit /b 1
